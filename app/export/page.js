...
export default function Export() {
  ...
  const exporterPDF = async () => {
    ...
    doc.setTextColor(29, 78, 216)
    doc.text(`${totalCA.toFixed(2)} euros`, 155, 83)
    doc.setTextColor(60, 60, 60)
    doc.text('Total des frais', 25, 93)
    doc.setTextColor(239, 68, 68)
    doc.text(`- ${totalFrais.toFixed(2)} euros`, 155, 93)
    doc.setTextColor(60, 60, 60)
    doc.text('Net estime', 25, 103)
    doc.setTextColor(22, 163, 74)
    doc.text(`${netEstime.toFixed(2)} euros`, 155, 103)
    ...
    if (donnees.factures.length === 0) {
      ...
      donnees.factures.forEach((f, i) => {
        ...
        doc.setTextColor(29, 78, 216)
        doc.text(`${Number(f.montant).toFixed(2)} euros`, 165, y + 2)
        y += 8
      })
      ...
      doc.text(`${totalCA.toFixed(2)} euros`, 165, y + 5.5)
      y += 12
    }
    ...
    if (donnees.frais.length === 0) {
      ...
      doc.text(`- ${totalFrais.toFixed(2)} euros`, 165, y + 5.5)
      y += 12
    }

    doc.setFontSize(8)
    doc.setTextColor(150, 150, 150)
    doc.text(`Genere le ${new Date().toLocaleDateString('fr-FR')} — zimvu.app`, 20, 285)
  ...
  const exporterExcel = async () => {
    ...
    const facturesRows = [
      ['Date', 'Client', 'Description', 'Statut', 'Montant (euros)'],
    ...
    wsFactures['!cols'] = [{ wch: 12 }, { wch: 25 }, { wch: 35 }, { wch: 15 }, { wch: 15 }]
    XLSX.utils.book_append_sheet(wb, wsFactures, 'Factures')

    const fraisRows = [
      ['Date', 'Categorie', 'Description', 'Statut', 'Montant (euros)'],
    ...
    wsFrais['!cols'] = [{ wch: 12 }, { wch: 20 }, { wch: 35 }, { wch: 15 }, { wch: 15 }]
        ...
        {isPlanGratuit && (
          <div className="bg-orange-50 border border-orange-200 rounded-2xl p-6 mb-6 text-center">
            ...
            <p className="text-orange-600 text-sm mb-4">
              L'export comptable est disponible avec le plan Pro.
            </p>
        ...
        {!isPlanGratuit && (
          <>
            <div className="bg-white rounded-2xl shadow p-4 md:p-6 mb-6">
              <h3 className="text-sm font-semibold text-gray-600 mb-4">Selectionner la periode</h3>
              <div className="flex gap-4 flex-wrap">
                <select value={moisSelectionne}
                ...
                <select value={anneeSelectionnee}
              ...
              <>
                <div className="grid grid-cols-1 md:grid-cols-3 gap-4 mb-6">
                  <div className="bg-white rounded-2xl shadow p-4 text-center">
                    ...
                    <p className="text-2xl font-bold text-blue-700">{totalCA.toFixed(2)} euros</p>
                  ...
                  <div className="bg-white rounded-2xl shadow p-4 text-center">
                    ...
                    <p className="text-2xl font-bold text-red-500">{totalFrais.toFixed(2)} euros</p>
                    <p className="text-xs text-gray-400 mt-1">{donnees.frais.length} note(s)</p>
                  </div>
                  <div className="bg-white rounded-2xl shadow p-4 text-center">
                    <p className="text-gray-500 text-sm mb-1">Net estime</p>
                    <p className={`text-2xl font-bold ${netEstime >= 0 ? 'text-green-600' : 'text-red-500'}`}>
                      {netEstime.toFixed(2)} euros
                    </p>
                ...
                <div className="bg-white rounded-2xl shadow p-4 md:p-6 mb-4">
                  <h3 className="text-sm font-semibold text-gray-600 mb-3">
                    Factures ({donnees.factures.length}) — hors annulees
                  </h3>
                        ...
                        <div key={f.id} className="flex justify-between items-center py-2 border-b border-gray-50">
                          ...
                          <div className="flex items-center gap-3">
                            <span className={`px-2 py-0.5 rounded-full text-xs font-semibold ${
                              f.statut === 'Payee' ? 'bg-green-100 text-green-700' : 'bg-yellow-100 text-yellow-700'
                            }`}>{f.statut}</span>
                            <span className="font-semibold text-blue-700 text-sm">{Number(f.montant).toFixed(2)} euros</span>
                          </div>
                        </div>
                      ))}
                    </div>
                    ...
                    <div className="space-y-2">
                      {donnees.frais.map(f => (
                        <div key={f.id} className="flex justify-between items-center py-2 border-b border-gray-50">
                          ...
                          <span className="font-semibold text-red-500 text-sm">- {Number(f.montant).toFixed(2)} euros</span>
                        </div>
                      ))}
                    </div>
                  )}
