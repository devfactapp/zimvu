...
const TYPES = [
  { label: 'Echeance facture', emoji: '💰', couleur: 'bg-blue-100 text-blue-700' },
  ...
  { label: 'Tache', emoji: '✅', couleur: 'bg-gray-100 text-gray-700' },
  { label: 'Declaration URSSAF', emoji: '🏛️', couleur: 'bg-red-100 text-red-700' },
]
...
export default function Agenda() {
  ...
  const toggleStatut = async (id, statut) => {
    const nouveau = statut === 'A faire' ? 'Fait' : 'A faire'
  ...
  const evenementsAVenir = evenements.filter(e => {
    const d = new Date(e.date)
    return d >= aujourdhui && d <= dans30jours && e.statut === 'A faire'
      ...
      {confirmSupprimer && (
        <div style={{position:'fixed', inset:0, backgroundColor:'rgba(0,0,0,0.5)', display:'flex', alignItems:'center', justifyContent:'center', zIndex:50, padding:'0 16px'}}>
          <div className="bg-white rounded-2xl shadow-2xl p-6 w-full max-w-sm">
            <h3 className="text-lg font-semibold text-gray-800 mb-2">Supprimer cet evenement ?</h3>
            <p className="text-gray-500 text-sm mb-6"><strong>{confirmSupprimer.titre}</strong> sera supprime definitivement.</p>
        ...
        <div style={{position:'fixed', inset:0, backgroundColor:'rgba(0,0,0,0.5)', display:'flex', alignItems:'center', justifyContent:'center', zIndex:50, padding:'0 16px'}}>
          <div className="bg-white rounded-2xl shadow-2xl p-6 w-full max-w-md max-h-screen overflow-y-auto">
            <div className="flex items-center justify-between mb-4">
              ...
              <span onClick={() => setJourSelectionne(null)} className="text-gray-400 cursor-pointer hover:text-gray-600 text-xl">x</span>
            </div>

            {jourSelectionnéEvenements.length === 0 ? (
              <p className="text-gray-400 text-sm mb-4">Aucun evenement ce jour</p>
            ) : (
        ...
        {isPlanGratuit && (
          <div className="bg-orange-50 border border-orange-200 rounded-2xl p-6 mb-6 text-center">
            ...
            <p className="text-orange-600 text-sm mb-4">
              L'agenda est disponible avec le plan Pro.
            </p>
          ...
          <>
            {showForm && (
              <div className="bg-white rounded-2xl shadow p-4 md:p-6 mb-6">
                <h3 className="text-lg font-semibold text-gray-700 mb-4">Nouvel evenement</h3>
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4 mb-4">
                  <div className="md:col-span-2">
                    <label className="block text-sm font-medium text-gray-700 mb-1">Titre *</label>
                    <input type="text" placeholder="Ex: Relancer M. Dupont"
                      value={form.titre}
                  ...
                  <div className="md:col-span-2">
                    ...
                    <input type="text" placeholder="Details optionnels..."
            ...
            <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
              ...
              <div className="bg-white rounded-2xl shadow p-4 md:p-6">
                <h3 className="font-semibold text-gray-700 mb-4">A venir (30 jours)</h3>
                ...
                <div className="mt-6 pt-4 border-t border-gray-100">
                  ...
                  <p className="text-sm text-gray-600">{evenementsMois.length} evenement(s)</p>
                  <p className="text-sm text-green-600">{evenementsMois.filter(e => e.statut === 'Fait').length} termine(s)</p>
                  <p className="text-sm text-yellow-600">{evenementsMois.filter(e => e.statut === 'A faire').length} a faire</p>
                </div>
              </div>
            </div>
          </>
